...
controller.hears(['stats'], 'direct_message,direct_mention,mention', function(bot, message) {
	...
	bot.api.channels.history({token: process.env.app_token,channel: bots_channel_id, count: 1000, oldest: ts_start, latest: ts_end }
...
controller.hears(['.*'], 'direct_message,direct_mention,mention',function(bot, message) {
	...
	//@ https://api.slack.com/methods/channels.history
	bot.api.channels.history({token: process.env.app_token,channel: bots_channel_id, count: count, oldest: ts_start, latest: ts_end }
		, function(error, response){
    var erro = false;
		if(response.messages !== undefined){
			response.messages.forEach(function (result) {
